...
def cancelar_nota():
    try:
        ...
        notas_activas = cursor.fetchall()  # Usar fetchall para obtener todas las filas
...
def servicios_mas_prestados():
    try:
        ...
        # Consulta SQL para obtener los servicios más prestados en el período
        consulta_sql = f"""
...
def clientes_con_mas_notas():
    try:
        ...
        # Consulta SQL para obtener los clientes con más notas en el período
        consulta_sql = f"""
...
def promedio_montos_notas():
    try:
        ...
        df_notas['Fecha'] = pd.to_datetime(df_notas['Fecha'], format="%Y-%m-%d").dt.date
